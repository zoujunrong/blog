    ...
    Radar.prototype.draw = function() {
      ...
      for(var i = min; i <= max; i += this.settings.step) {
        ...
        this.cxt.arc(this.width/2,
                     ...
                     0, 2 * Math.PI, false);
        this.cxt.strokeStyle = "silver";
        this.cxt.fillStyle = "silver";
        this.cxt.stroke();
      ...
      i = 0;
      var maxX = that.width / 2 + Math.cos((Math.PI * 2) * (i / size)) * spacing * max;
      // var maxY = that.height / 2 + Math.sin((Math.PI * 2) * (i / size)) * spacing * max;

      $.each(this.settings.values, function(key,val){
        that.newCanvas('label-'+i, i * 250);
        that.cxt.fillStyle = "rgba(0,0,0,.8)";
        that.cxt.strokeStyle = "rgba(0,0,0,.5)";
        that.cxt.font = "bold 10px Verdana";


        var textX = that.width / 2 + Math.cos((Math.PI * 2) * (i / size)) * spacing * max;
        var textY = that.height / 2 + Math.sin((Math.PI * 2) * (i / size)) * spacing * max;

        var pos = maxX - textX + 30

        if ( pos < that.cxt.measureText(key).width ) {
          textX -= that.cxt.measureText(key).width - pos
        }
